...
START_TEST(s21_sum_matrix_2) {
  ...
  s21_create_matrix(5, 5, &B);
  ck_assert_int_eq(s21_sum_matrix(&A, &B, &result), CALCULATION_ERR);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
...
START_TEST(s21_mult_number_2) {
  ...
  s21_create_matrix(3, 3, &A);
  s21_initialize_matrix(&A, 1, 1);
  ck_assert_int_eq(s21_mult_number(&A, number, &result), CALCULATION_ERR);
  s21_remove_matrix(&A);
}
END_TEST
...
START_TEST(s21_determinant_2) {
  ...
  s21_create_matrix(3, 2, &A);
  s21_initialize_matrix(&A, 1, 1);
  ck_assert_int_eq(s21_determinant(&A, &det), CALCULATION_ERR);
  s21_remove_matrix(&A);
}
END_TEST
